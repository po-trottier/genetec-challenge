...
import { translate, Constants, retry, RetryOperationType, ConditionErrorNameMapper, ErrorNameConditionMapper } from "@azure/amqp-common";
...
export class MessageReceiver extends LinkEntity {
    constructor(context, receiverType, options) {
        ...
        this._clearMessageLockRenewTimer = (messageId) => {
            if (this._messageRenewLockTimers.has(messageId)) {
                ...
                log.receiver("[%s] Cleared the message renew lock timer for message with id '%s'.", this._context.namespace.connectionId, messageId);
                this._messageRenewLockTimers.delete(messageId);
            }
        };
        this._clearAllMessageLockRenewTimers = () => {
            log.receiver("[%s] Clearing message renew lock timers for all the active messages.", this._context.namespace.connectionId);
        ...
        this._onAmqpMessage = (context) => __awaiter(this, void 0, void 0, function* () {
            ...
            if (this.autoRenewLock && bMessage.lockToken) {
                ...
                log.receiver("[%s] message with id '%s' is locked until %s.", connectionId, bMessage.messageId, bMessage.lockedUntilUtc.toString());
                const totalAutoLockRenewDuration = Date.now() + this.maxAutoRenewDurationInSeconds * 1000;
                log.receiver("[%s] Total autolockrenew duration for message with id '%s' is: ", connectionId, bMessage.messageId, new Date(totalAutoLockRenewDuration).toString());
                    ...
                    if (new Date(totalAutoLockRenewDuration) > bMessage.lockedUntilUtc &&
                        ...
                        if (this._messageRenewLockTimers.has(bMessage.messageId)) {
                            ...
                            this._messageRenewLockTimers.set(bMessage.messageId, setTimeout(() => __awaiter(this, void 0, void 0, function* () {
                                try {
                                    log.receiver("[%s] Attempting to renew the lock for message with id '%s'.", connectionId, bMessage.messageId);
                                    bMessage.lockedUntilUtc = yield this._context.managementClient.renewLock(lockToken);
                                    log.receiver("[%s] Successfully renewed the lock for message with id '%s'.", connectionId, bMessage.messageId);
                                    log.receiver("[%s] Calling the autorenewlock task again for message with " + "id '%s'.", connectionId, bMessage.messageId);
                                    autoRenewLockTask();
                                }
                                catch (err) {
                                    log.error("[%s] An error occured while auto renewing the message lock '%s' " +
                            ...
                            log.receiver("[%s] Looks like the message lock renew timer has already been " +
                                "cleared for message with id '%s'.", connectionId, bMessage.messageId);
                        }
                    }
                    else {
                        log.receiver("[%s] Current time %s exceeds the total autolockrenew duration %s for " +
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                if (!this.isOpen() && !this.isConnecting) {
                    ...
                    log.error("[%s] Receiver '%s' with address '%s' has established itself.", connectionId, this.name, this.address);
                ...
                this.isConnecting = false;
                err = translate(err);
                log.error("[%s] An error occured while creating the receiver '%s': %O", this._context.namespace.connectionId, this.name, err);
                throw err;
            }
        });
    }
    _deleteFromCache() {
        ...
        log.error("[%s] Deleted the receiver '%s' from the client cache.", this._context.namespace.connectionId, this.name);
    }
    /**
    ...
    onDetached(receiverError, causedByDisconnect) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (this._isDetaching) {
                ...
                log.error(`[${connectionId}] Call to detached on streaming receiver '${this.name}' is already in progress.`);
                return;
            }
            ...
            try {
                ...
                if (this.receiverType === ReceiverType.batching) {
                    log.error("[%s] Receiver '%s' with address '%s' is a Batching Receiver, so we will not be " +
                ...
                if (!shouldReopen && !causedByDisconnect) {
                    log.error("[%s] Encountered a non retryable error on the receiver. Cannot recover receiver '%s' with address '%s' encountered error: %O", connectionId, this.name, this.address, translatedError);
                    return;
                }
                ...
                if (!shouldReopen && causedByDisconnect) {
                    log.error("[%s] Encountered a non retryable error on the connection. Cannot recover receiver '%s' with address '%s': %O", connectionId, this.name, this.address, translatedError);
                ...
                const config = {
                    operation: () => this._init(options).then(() => __awaiter(this, void 0, void 0, function* () {
                        if (this.wasCloseInitiated) {
                            log.error("[%s] close() method of Receiver '%s' with address '%s' was called. " +
                ...
                log.error("[%s] An error occurred while processing detached() of Receiver '%s': %O ", connectionId, this.name, this.address, err);
                if (typeof this._onError === "function") {
                    log.error("[%s] Unable to automatically reconnect Receiver '%s' with address '%s'.", connectionId, this.name, this.address);
                    try {
                        this._onError(err);
                    }
                    catch (err) {
                        log.error("[%s] User-code error in error handler called after disconnect: %O", connectionId, err);
                    }
                    finally {
    ...
    close() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            log.receiver("[%s] Closing the [%s]Receiver for entity '%s'.", this._context.namespace.connectionId, this.receiverType, this._context.entityPath);
            if (this._newMessageReceivedTimer)
                clearTimeout(this._newMessageReceivedTimer);
            this._clearAllMessageLockRenewTimers();
            if (this._receiver) {
    ...
    isOpen() {
        ...
        log.error("[%s] Receiver '%s' with address '%s' is open? -> %s", this._context.namespace.connectionId, this.name, this.address, result);
        return result;
    }
}
