    ...
    receive(maxMessageCount, maxWaitTimeInSeconds) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                log.error("[%s] Receiver '%s': Rejecting receiveMessages() with error %O: ", this._context.namespace.connectionId, this.name, error);
                throw error;
            }
            finally {
                this.isReceivingMessages = false;
        ...
        return new Promise((resolve, reject) => {
            ...
            this._connectionErrorHandler = (error) => {
                ...
                if (this.receiveMode === ReceiveMode.receiveAndDelete && brokeredMessages.length) {
                    log.batching("[%s] Receiver '%s': Connection disconnected. Resolving receiveMessages() with %d messages.", this._context.namespace.connectionId, this.name, brokeredMessages.length);
            ...
            const finalAction = () => {
                ...
                if (this._receiver && this._receiver.isOpen() && this._receiver.credit > 0) {
                    log.batching("[%s] Receiver '%s': Draining leftover credits(%d).", this._context.namespace.connectionId, this.name, this._receiver.credit);
                    ...
                    log.batching("[%s] Receiver '%s': Resolving receiveMessages() with %d messages.", this._context.namespace.connectionId, this.name, brokeredMessages.length);
            ...
            const onSessionClose = (context) => __awaiter(this, void 0, void 0, function* () {
                try {
                    const sessionError = context.session && context.session.error;
                    if (sessionError) {
                        log.error("[%s] 'session_close' event occurred for receiver '%s'. The associated error is: %O", this._context.namespace.connectionId, this.name, sessionError);
                    }
                }
                catch (err) {
            ...
            const onReceiveDrain = () => {
                ...
                log.batching("[%s] Receiver '%s' drained. Resolving receiveMessages() with %d messages.", this._context.namespace.connectionId, this.name, brokeredMessages.length);
                resolve(brokeredMessages);
            };
            const onReceiveClose = (context) => __awaiter(this, void 0, void 0, function* () {
                try {
                    ...
                    if (receiverError) {
                        log.error("[%s] 'receiver_close' event occurred. The associated error is: %O", this._context.namespace.connectionId, receiverError);
                    }
                }
                catch (err) {
            ...
            if (!this.isOpen()) {
                log.batching("[%s] Receiver '%s', setting max concurrent calls to 0.", this._context.namespace.connectionId, this.name);
