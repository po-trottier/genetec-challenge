...
import { RestError, stripRequest, stripResponse, stringifyXML, parseXML } from "@azure/core-http";
 ...
 * @param serializer
 */
export function executeAtomXmlOperation(serviceBusAtomManagementClient, webResource, serializer) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            if (response.bodyAsText) {
                response.parsedBody = yield parseXML(response.bodyAsText, { includeRoot: true });
            }
        }
        catch (err) {
            const error = new RestError(`Error occurred while parsing the response body - expected the service to return valid xml content.`, RestError.PARSE_ERROR, response.status, stripRequest(response.request), stripResponse(response));
 ...
 */
export function deserializeAtomXmlResponse(nameProperties, response) {
    return __awaiter(this, void 0, void 0, function* () {
...
function parseAtomResult(response, nameProperties) {
    ...
    log.error("Failure in parsing response body from service. Expected response to be in Atom XML format and have either feed or entry components, but received - %0", atomResponseInJson);
    throw new RestError("Error occurred while parsing the response body - expected the service to return atom xml content with either feed or entry elements.", RestError.PARSE_ERROR, response.status, stripRequest(response.request), stripResponse(response));
}
/**
 * @internal
 * @ignore
...
export function buildError(response) {
    if (!isKnownResponseCode(response.status)) {
        throw new RestError(`Service returned an error response with an unrecognized HTTP status code - ${response.status}`, "ServiceError", response.status, stripRequest(response.request), stripResponse(response));
    }
    ...
    const error = new RestError(errorMessage, errorCode, response.status, stripRequest(response.request), stripResponse(response));
    return error;
}
/**
 * @internal
