...
import { messageProperties, SenderEvents, message as RheaMessageUtil, generate_uuid } from "rhea-promise";
import { defaultLock, retry, translate, ErrorNameConditionMapper, RetryOperationType, Constants, randomNumberFromInterval, delay } from "@azure/amqp-common";
import { toAmqpMessage, getMessagePropertyTypeMismatchError } from "../serviceBusMessage";
...
export class MessageSender extends LinkEntity {
    ...
    constructor(context) {
         ...
         * @readonly
         */
        this.openLock = `sender-${generate_uuid()}`;
        this._onAmqpError = (context) => {
            const senderError = context.sender && context.sender.error;
            if (senderError) {
                const err = translate(senderError);
                log.error("[%s] An error occurred for sender '%s': %O.", this._context.namespace.connectionId, this.name, err);
            }
        };
        this._onSessionError = (context) => {
            ...
            if (sessionError) {
                ...
                log.error("[%s] An error occurred on the session of sender '%s': %O.", this._context.namespace.connectionId, this.name, err);
            }
        };
    ...
    _deleteFromCache() {
        ...
        log.error("[%s] Deleted the sender '%s' with address '%s' from the client cache.", this._context.namespace.connectionId, this.name, this.address);
    }
    ...
    _trySend(encodedMessage, sendBatch) {
        const sendEventPromise = () => new Promise((resolve, reject) => __awaiter(this, void 0, void 0, function* () {
            try {
                yield this.open();
                let waitTimer;
                log.sender("[%s] Sender '%s', credit: %d available: %d", this._context.namespace.connectionId, this.name, this._sender.credit, this._sender.session.outgoing.available());
                if (!this._sender.sendable()) {
                    log.sender("[%s] Sender '%s', waiting for 1 second for sender to become sendable", this._context.namespace.connectionId, this.name);
                    yield delay(1000);
                    log.sender("[%s] Sender '%s' after waiting for a second, credit: %d available: %d", this._context.namespace.connectionId, this.name, this._sender.credit, this._sender.session.outgoing.available());
                }
                if (this._sender.sendable()) {
                    let onRejected;
                    let onReleased;
                    ...
                    onAccepted = (context) => {
                        ...
                        removeListeners();
                        log.sender("[%s] Sender '%s', got event accepted.", this._context.namespace.connectionId, this.name);
                        resolve();
                    };
                    onRejected = (context) => {
                        removeListeners();
                        log.error("[%s] Sender '%s', got event rejected.", this._context.namespace.connectionId, this.name);
                        const err = translate(context.delivery.remote_state.error);
                        reject(err);
                    };
                    onReleased = (context) => {
                        removeListeners();
                        log.error("[%s] Sender '%s', got event released.", this._context.namespace.connectionId, this.name);
                        let err;
                    ...
                    onModified = (context) => {
                        removeListeners();
                        log.error("[%s] Sender '%s', got event modified.", this._context.namespace.connectionId, this.name);
                        let err;
                        if (context.delivery.remote_state.error) {
                            err = translate(context.delivery.remote_state.error);
                        }
                    ...
                    try {
                        ...
                        log.sender("[%s] Sender '%s', sent message with delivery id: %d", this._context.namespace.connectionId, this.name, delivery.id);
                    }
                    catch (error) {
                        removeListeners();
                        return reject(error);
    ...
    open(options) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            log.sender("Acquiring lock %s for initializing the session, sender and possibly the connection.", this.openLock);
            return defaultLock.acquire(this.openLock, () => __awaiter(this, void 0, void 0, function* () {
                try {
                    ...
                    if (!this.isOpen()) {
                        ...
                        log.error("[%s] Trying to create sender '%s'...", this._context.namespace.connectionId, this.name);
                        if (!options) {
                            options = this._createSenderOptions({});
                        }
                        this._sender = yield this._context.namespace.connection.createSender(options);
                        this.isConnecting = false;
                        log.error("[%s] Sender '%s' with address '%s' has established itself.", this._context.namespace.connectionId, this.name, this.address);
                        this._sender.setMaxListeners(1000);
                        log.error("[%s] Promise to create the sender resolved. Created sender with name: %s", this._context.namespace.connectionId, this.name);
                        log.error("[%s] Sender '%s' created with sender options: %O", this._context.namespace.connectionId, this.name, options);
                    ...
                    err = translate(err);
                    log.error("[%s] An error occurred while creating the sender %s", this._context.namespace.connectionId, this.name, err);
                    throw err;
                }
            }));
        });
        ...
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                if (senderError && !wasCloseInitiated) {
                    ...
                    log.error("[%s] Something went wrong. State of sender '%s' with address '%s' is: %O", this._context.namespace.connectionId, this.name, this.address, state);
                }
                if (shouldReopen) {
    ...
    close() {
        return __awaiter(this, void 0, void 0, function* () {
            if (this._sender) {
                log.sender("[%s] Closing the Sender for the entity '%s'.", this._context.namespace.connectionId, this._context.entityPath);
    ...
    isOpen() {
        ...
        log.error("[%s] Sender '%s' with address '%s' is open? -> %s", this._context.namespace.connectionId, this.name, this.address, result);
        return result;
    }
    /**
    ...
    send(data) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                log.sender("[%s] Sender '%s', trying to send message: %O", this._context.namespace.connectionId, this.name, data);
                return yield this._trySend(encodedMessage);
            }
            catch (err) {
                log.error("[%s] Sender '%s': An error occurred while sending the message: %O\nError: %O", this._context.namespace.connectionId, this.name, data, err);
                throw err;
            }
        });
    }
    ...
    sendBatch(inputMessages) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                log.sender("[%s] Sender '%s', trying to send Message[]: %O", this._context.namespace.connectionId, this.name, inputMessages);
                const amqpMessages = [];
                ...
                log.sender("[%s]Sender '%s', sending encoded batch message.", this._context.namespace.connectionId, this.name, encodedBatchMessage);
                return yield this._trySend(encodedBatchMessage, true);
            }
            catch (err) {
                log.error("[%s] Sender '%s': An error occurred while sending the messages: %O\nError: %O", this._context.namespace.connectionId, this.name, inputMessages, err);
                throw err;
            }
        });
    }
